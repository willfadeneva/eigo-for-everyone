...
export default function TutorsPage() {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-[#0f172a] mb-1">Find your perfect English tutor</h1>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-5 lg:hidden scrollbar-none -mx-4 px-4">
        {SPECIALTIES.map((s) => (
          <Badge
            ...
            className={`cursor-pointer text-xs whitespace-nowrap flex-shrink-0 py-1.5 px-3 ${s === "All" ? "bg-[#8774DB] text-white" : "hover:bg-[#fce7f3] hover:text-[#8774DB]"}`}
          >
            {s}
          </Badge>
        ))}
      ...
      <div className="flex gap-8">
        ...
        <aside className="hidden lg:block w-56 flex-shrink-0">
          <div className="sticky top-24 space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {SPECIALTIES.map((s) => (
                  <Badge
                    key={s}
                    variant={s === "All" ? "default" : "secondary"}
                    className={`cursor-pointer text-xs ${s === "All" ? "bg-[#8774DB] text-white" : "hover:bg-[#fce7f3] hover:text-[#8774DB]"}`}
                  >
                    {s}
                  </Badge>
                ))}
            ...
            <Button className="w-full bg-[#49D1FD] hover:bg-[#8774DB] text-white text-sm">Apply filters</Button>
          </div>
        </aside>
          ...
          <div className="flex items-center justify-center gap-2 mt-10">
            ...
            {[1,2,3].map((p) => (
              <Button key={p} variant={p === 1 ? "default" : "outline"} size="sm"
                className={p === 1 ? "bg-[#8774DB] text-white" : ""}>
                {p}
              </Button>
            ))}
