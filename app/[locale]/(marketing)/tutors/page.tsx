...
export default async function TutorsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* ── Dark hero banner ── */}
      <div className="relative bg-[#0D0A1E] overflow-hidden py-14 px-4">
        <div className="absolute inset-0 bg-gradient-to-br from-[#5B21B6]/40 via-transparent to-[#EC4899]/20 pointer-events-none" />
        <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[200px] font-black text-white/5 select-none pointer-events-none italic hidden lg:block">T</div>
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="inline-flex items-center gap-2 bg-[#8774DB]/20 border border-[#8774DB]/40 rounded-full px-4 py-1.5 text-sm font-semibold text-[#C4B5FD] mb-4">
            ✦ 500+ verified native tutors
          </div>
          <h1 className="text-4xl sm:text-5xl font-black text-white mb-3">{t("title")}</h1>
          <p className="text-white/50">Trial lessons FREE · From ₹700/hr · IELTS · Business · Kids · Conversation</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-5">
          <div className="relative flex-1">
            ...
            <Input placeholder={t("searchPlaceholder")} className="pl-9 rounded-xl h-11" />
          </div>
          ...
          <Button variant="outline" className="flex items-center justify-center gap-2 rounded-xl h-11 border-[#e9d8fd] text-[#8774DB]">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 mb-5 lg:hidden scrollbar-none -mx-4 px-4">
          {SPECIALTIES.map(s => (
            <Badge key={s} variant={s === "All" ? "default" : "secondary"}
              className={`cursor-pointer text-xs whitespace-nowrap flex-shrink-0 py-1.5 px-3 rounded-full ${s === "All" ? "bg-[#8774DB] text-white" : "hover:bg-[#f5f3ff] hover:text-[#8774DB]"}`}>
              {s}
            </Badge>
          ))}
        </div>
        ...
        <div className="flex gap-8">
          ...
          <aside className="hidden lg:block w-56 flex-shrink-0">
            <div className="sticky top-24 space-y-6 bg-white rounded-2xl border border-[#f0ebff] p-5 shadow-sm">
              <div>
                <h3 className="font-bold text-[#0f172a] mb-3 text-sm">{t("filterSpecialty")}</h3>
                <div className="flex flex-wrap gap-1.5">
                  {SPECIALTIES.map(s => (
                    <Badge key={s} variant={s === "All" ? "default" : "secondary"}
                      className={`cursor-pointer text-xs rounded-full ${s === "All" ? "bg-[#8774DB] text-white" : "hover:bg-[#f5f3ff] hover:text-[#8774DB]"}`}>
                      {s}
                    </Badge>
                  ))}
                </div>
              ...
              <div>
                <h3 className="font-bold text-[#0f172a] mb-3 text-sm">{t("filterPrice")} (₹/50min)</h3>
                <div className="flex gap-2 items-center text-sm">
                  <Input placeholder="₹500" className="h-8 text-xs rounded-lg" />
                  <span className="text-slate-400">–</span>
                  <Input placeholder="₹5000" className="h-8 text-xs rounded-lg" />
                </div>
              </div>
              <div>
                <h3 className="font-bold text-[#0f172a] mb-3 text-sm">{t("filterRating")}</h3>
              ...
              <Button className="w-full bg-[#8774DB] hover:bg-[#7261C8] text-white text-sm rounded-xl font-bold">
                Apply filters
              </Button>
            </div>
          </aside>
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-[#64748b] mb-4 font-medium">{MOCK_TUTORS.length} tutors found</p>
            ...
            <div className="flex items-center justify-center gap-2 mt-12">
              <Button variant="outline" size="sm" disabled className="rounded-xl">← Prev</Button>
              {[1,2,3].map(p => (
                <Button key={p} variant={p === 1 ? "default" : "outline"} size="sm"
                  className={`rounded-xl ${p === 1 ? "bg-[#8774DB] text-white" : ""}`}>{p}</Button>
              ))}
