...
function StatusBadge({ s }: { s: string }) {
  ...
  return <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${m[s] ?? "bg-slate-100 text-[#5b5389]"}`}>{s}</span>;
}
    ...
    <div className="min-h-screen bg-[#fdf8ff]">
      <div className="bg-white border-b border-[#f0ebff] px-4 sm:px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Avatar className="w-8 h-8"><AvatarFallback className="bg-[#818cf8] text-white text-sm">S</AvatarFallback></Avatar>
        </div>
      </div>

      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-2xl font-bold text-[#1e1b4b] mb-6">Tutor Dashboard</h1>
        ...
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="overview" className="space-y-6">
              ...
              {[
                ...
                <Card key={label} className="border border-[#f0ebff]">
                  <CardContent className="p-4">
                    ...
                    <div className="font-bold text-lg text-[#1e1b4b]">{value}</div>
            ...
            <Card className="border border-[#f0ebff]">
              ...
              <CardContent className="p-4">
                {UPCOMING[0] ? (
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div>
                      ...
                      <div className="text-sm text-[#7c6f9e] mt-1"><Calendar size={12} className="inline mr-1"/>{UPCOMING[0].date} · {UPCOMING[0].duration} min</div>
                    </div>
                ...
                ) : <p className="text-[#7c6f9e] text-sm">No upcoming lessons.</p>}
              </CardContent>
            </Card>
          </TabsContent>
                    ...
                    <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2">
                        <Link href={`/classroom/${l.id}`}>
                          <Button size="sm" className="bg-[#818cf8] text-white"><Video size={13} className="mr-1"/>Start</Button>
                        </Link>
                  ...
                  <Card key={l.id} className="border border-[#f0ebff]">
                    <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="text-sm text-[#7c6f9e] mt-1"><Calendar size={12} className="inline mr-1"/>{l.date}</div>
                      </div>
                      <span className="font-semibold text-[#6366f1]">₹{l.amount.toLocaleString()}</span>
                    </CardContent>
                  </Card>
                ))}
              </div>
          ...
          <TabsContent value="availability">
            <div className="space-y-4">
              <p className="text-sm text-[#7c6f9e]">Click slots to toggle availability. Blue = available.</p>
              <div className="overflow-x-auto">
                <table className="border-collapse text-xs">
                  <thead>
                    <tr>
                      <th className="w-16 text-left text-slate-400 font-normal pb-2 pr-2">Time</th>
                      {DAYS.map(d => <th key={d} className="text-center text-[#5b5389] font-medium pb-2 px-2 w-16">{d}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {HOURS.map(h => (
                          ...
                          return (
                            <td key={d} className="px-2 py-0.5 text-center">
                              <button onClick={() => toggleSlot(d, h)}
                                className={`w-12 h-6 rounded transition-colors text-[10px] font-medium ${on ? "bg-[#818cf8] text-white" : "bg-slate-100 text-slate-400 hover:bg-slate-200"}`}>
                                {on ? "Open" : ""}
                              </button>
                            </td>
                          );
              ...
              <Button className="bg-[#818cf8] text-white" onClick={() => alert("Availability saved! (TODO: sync to DB)")}>
                Save availability
              </Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="earnings" className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                <Card key={label} className="border border-[#f0ebff]">
                  <CardContent className="p-4">
                    <div className="text-xs text-[#7c6f9e] mb-1">{label}</div>
                    <div className={`text-xl font-bold ${highlight ? "text-[#6366f1]" : "text-slate-800"}`}>{value}</div>
            ...
            <div>
              ...
              <div className="rounded-xl border border-[#f0ebff] overflow-hidden">
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {TRANSACTIONS.map((t, i) => (
                      <TableRow key={i}>
                        <TableCell className="text-[#7c6f9e] text-sm">{t.date}</TableCell>
                        ...
                        <TableCell className="text-right text-sm font-semibold text-[#6366f1]">₹{t.payout}</TableCell>
          ...
          <TabsContent value="messages">
            <div className="flex flex-col sm:flex-row border border-[#e9d8fd] rounded-xl overflow-hidden h-auto sm:h-[520px] bg-white">
              <div className="w-60 border-r border-[#f0ebff] flex flex-col flex-shrink-0">
                <div className="p-3 border-b border-[#f0ebff] font-semibold text-slate-700 text-sm">Students</div>
                {CONVERSATIONS.map((c, i) => (
                  <div key={c.student} className={`p-3 cursor-pointer hover:bg-[#faf8ff] flex items-start gap-2 ${i === 0 ? "bg-[#ede9fe]" : ""}`}>
                    <Avatar className="w-8 h-8 flex-shrink-0"><AvatarFallback className="bg-[#818cf8] text-white text-xs">{c.student.charAt(0)}</AvatarFallback></Avatar>
              ...
              <div className="flex-1 flex flex-col">
                <div className="p-3 border-b border-[#f0ebff] font-semibold text-slate-800 text-sm">Priya Sharma</div>
                ...
                <form onSubmit={sendMsg} className="p-3 border-t border-[#f0ebff] flex gap-2">
                  <Input value={msgInput} onChange={e => setMsgInput(e.target.value)} placeholder="Type a message…" className="flex-1 text-sm"/>
                  <Button size="sm" type="submit" className="bg-[#818cf8] text-white"><Send size={14}/></Button>
                </form>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="profile">
            <Card className="border border-[#f0ebff] max-w-2xl">
              <CardContent className="p-6 space-y-4">
                ...
                <Button className="bg-[#818cf8] text-white">Save profile</Button>
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
