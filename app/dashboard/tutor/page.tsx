    ...
    <div className="min-h-screen bg-[#ffffff]">
      <div className="bg-white border-b border-[#fdf2f8] px-4 sm:px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Avatar className="w-8 h-8"><AvatarFallback className="bg-[#D946EF] text-white text-sm">S</AvatarFallback></Avatar>
        </div>
      </div>
        ...
        <Tabs defaultValue="overview">
                ...
                <Card key={label} className="border border-[#fdf2f8]">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2 mb-1">
                      <Icon size={16} className="text-[#D946EF]"/>
                    ...
                    <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2">
                        <Link href={`/classroom/${l.id}`}>
                          <Button size="sm" className="bg-[#D946EF] text-white"><Video size={13} className="mr-1"/>Start</Button>
                        </Link>
                ...
                {PAST.map(l => (
                  <Card key={l.id} className="border border-[#fdf2f8]">
                    <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <span className="font-semibold text-[#D946EF]">₹{l.amount.toLocaleString()}</span>
                    </CardContent>
                  </Card>
                ))}
              </div>
          ...
          <TabsContent value="availability">
            <div className="space-y-4">
                          ...
                          return (
                            <td key={d} className="px-2 py-0.5 text-center">
                              <button onClick={() => toggleSlot(d, h)}
                                className={`w-12 h-6 rounded transition-colors text-[10px] font-medium ${on ? "bg-[#D946EF] text-white" : "bg-slate-100 text-slate-400 hover:bg-slate-200"}`}>
                                {on ? "Open" : ""}
                              </button>
                            </td>
                          );
              ...
              <Button className="bg-[#D946EF] text-white" onClick={() => alert("Availability saved! (TODO: sync to DB)")}>
                Save availability
              </Button>
            </div>
          </TabsContent>
              ...
              {[
                ...
                <Card key={label} className="border border-[#fdf2f8]">
                  <CardContent className="p-4">
                    ...
                    <div className={`text-xl font-bold ${highlight ? "text-[#D946EF]" : "text-slate-800"}`}>{value}</div>
                  ...
                  <TableBody>
                    {TRANSACTIONS.map((t, i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell className="text-right text-sm font-semibold text-[#D946EF]">₹{t.payout}</TableCell>
            ...
            <div className="flex flex-col sm:flex-row border border-[#fce7f3] rounded-xl overflow-hidden h-auto sm:h-[520px] bg-white">
              <div className="w-60 border-r border-[#fdf2f8] flex flex-col flex-shrink-0">
                ...
                {CONVERSATIONS.map((c, i) => (
                  <div key={c.student} className={`p-3 cursor-pointer hover:bg-[#fafafa] flex items-start gap-2 ${i === 0 ? "bg-[#fce7f3]" : ""}`}>
                    <Avatar className="w-8 h-8 flex-shrink-0"><AvatarFallback className="bg-[#D946EF] text-white text-xs">{c.student.charAt(0)}</AvatarFallback></Avatar>
              ...
              <div className="flex-1 flex flex-col">
                ...
                <form onSubmit={sendMsg} className="p-3 border-t border-[#fdf2f8] flex gap-2">
                  ...
                  <Button size="sm" type="submit" className="bg-[#D946EF] text-white"><Send size={14}/></Button>
                </form>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="profile">
            <Card className="border border-[#fdf2f8] max-w-2xl">
              <CardContent className="p-6 space-y-4">
                ...
                <Button className="bg-[#D946EF] text-white">Save profile</Button>
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
