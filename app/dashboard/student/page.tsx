    ...
    <div className="min-h-screen bg-[#ffffff]">
      ...
      <div className="bg-white border-b border-[#fdf2f8] px-4 sm:px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Avatar className="w-8 h-8"><AvatarFallback className="bg-[#8774DB] text-white text-sm">P</AvatarFallback></Avatar>
        ...
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="overview" className="space-y-6">
                ...
                <Card key={label} className="border border-[#fdf2f8]">
                  <CardContent className="p-4 flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg bg-[#fce7f3] flex items-center justify-center flex-shrink-0">
                      <Icon size={18} className="text-[#8774DB]" />
                    </div>
            ...
            <Card className="border border-[#fdf2f8]">
              ...
              <CardContent className="p-4">
                ...
                ) : <p className="text-[#64748b] text-sm">No upcoming lessons. <Link href="/tutors" className="text-[#8774DB] underline">Find a tutor</Link>.</p>}
              </CardContent>
            </Card>
                  ...
                  <Card key={l.id} className="border border-[#fdf2f8]">
                    <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <Link href={`/classroom/${l.id}`}>
                        <Button size="sm" className="bg-[#8774DB] text-white"><Video size={13} className="mr-1"/>Join</Button>
                      </Link>
                    </CardContent>
                  </Card>
                ))}
                ...
                {PAST.map(l => (
                  <Card key={l.id} className="border border-[#fdf2f8]">
                    <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      {!l.reviewed ? (
                        <Button size="sm" variant="outline" className="border-[#8774DB] text-[#8774DB]">
          ...
          <TabsContent value="tutors">
            <div className="mb-4 flex gap-3">
              ...
              <Link href="/tutors"><Button className="bg-[#8774DB] text-white">Browse all</Button></Link>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {RECOMMENDED.map(t => <TutorCard key={t.id} {...t} />)}
            </div>
            ...
            <div className="flex flex-col sm:flex-row border border-[#fce7f3] rounded-xl overflow-hidden h-auto sm:h-[520px] bg-white">
              ...
              <div className="w-60 border-r border-[#fdf2f8] flex flex-col flex-shrink-0">
                ...
                {CONVERSATIONS.map((c, i) => (
                  <div key={c.tutor} className={`p-3 cursor-pointer hover:bg-[#fafafa] flex items-start gap-2 ${i === 0 ? "bg-[#fce7f3]" : ""}`}>
                    <Avatar className="w-8 h-8 flex-shrink-0"><AvatarFallback className="bg-[#8774DB] text-white text-xs">{c.tutor.charAt(0)}</AvatarFallback></Avatar>
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div className="p-3 border-t border-[#fdf2f8] flex gap-2">
                  ...
                  <Button size="sm" className="bg-[#8774DB] text-white"><Send size={14}/></Button>
                </div>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="settings">
            <Card className="border border-[#fdf2f8] max-w-lg">
              <CardContent className="p-6 space-y-4">
                ...
                <Button className="bg-[#8774DB] text-white">Save changes</Button>
                ...
                <Button variant="outline" className="border-[#8774DB] text-[#8774DB]">Update password</Button>
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
