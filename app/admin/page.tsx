                ...
                <div key={t.name} className="flex items-center justify-between bg-white rounded-lg p-3 border border-amber-100">
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <Link href={`/admin/tutors/${encodeURIComponent(t.name.toLowerCase().replace(/ /g,"-"))}`}>
                      <Button size="sm" className="bg-[#8774DB] text-white text-xs h-7">Review</Button>
                    </Link>
                  </div>
                </div>
              ))}
              ...
              <TableBody>
                {RECENT_SIGNUPS.map(u => (
                  <TableRow key={u.email}>
                    ...
                    <TableCell>
                      <Badge className={u.role === "TUTOR" ? "bg-[#fce7f3] text-[#8774DB] text-xs" : "bg-slate-100 text-[#475569] text-xs"}>
                        {u.role}
                      </Badge>
                    </TableCell>
