...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#0f172a]">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm">
          <div className="text-center mb-6">
            <div className="text-3xl font-bold text-[#8774DB]">英語 Eigo</div>
            <div className="text-[#64748b] text-sm mt-1">Admin Console</div>
          </div>
          <form onSubmit={login} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full border border-[#fce7f3] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#8774DB]"
            ...
            <button type="submit"
              className="w-full bg-[#49D1FD] hover:bg-[#8774DB] text-white font-semibold py-2.5 rounded-lg text-sm transition-colors">
              Sign in
            </button>
          </form>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#ffffff]">
      <aside className="w-full md:w-56 bg-[#0f172a] text-white flex flex-col flex-shrink-0">
            ...
            return (
              <Link key={href} href={href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                  active ? "bg-[#8774DB] text-white" : "text-pink-200 hover:bg-white/10 hover:text-white"
                }`}>
                <Icon size={16}/>
                {label}
